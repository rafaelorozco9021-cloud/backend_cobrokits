...
export class CustomersController {
  ...
  async list(@Query('sellerId') sellerId?: string, @Req() req?: any) {
    try {
      ...
      if (sellerId) return this.dataSource.query('SELECT * FROM cobrokits.customers WHERE seller_id = $1 ORDER BY created_at DESC LIMIT 100', [sellerId]);
      return this.dataSource.query('SELECT * FROM cobrokits.customers ORDER BY created_at DESC LIMIT 100');
    } catch (e) {
  ...
  async create(@Body() body: any, @Req() req?: any) {
    try {
      ...
      // También si viene seller_id pero no cobro_id, no hacer nada
      const id = body.id;
      if (id) {
        ...
        const rows: any[] = await this.dataSource.query(`SELECT * FROM cobrokits.customers WHERE id=$1`, [id]);
        return rows[0];
      }
